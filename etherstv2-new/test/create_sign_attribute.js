...
const TEST_DATA_BYTE_COUNT = 500;

const APPROVED_AMOUNT = 50;

const TOTAL_ACCOUNT_COUNT = 100;
const SIM_ACCOUNT_COUNT = 50;
...
function generateRandomAttribute() {
    ...
    let randomData = generateRandomString(10);
...
async function createAttribute(account) {
    ...
    for(let i=0; i< randomAttributesCount;++i) {
        ...
        let attr = await this.ethrust.createAttribute(a['type'], /*a['has_proof'], */ a['id'], a['data'], "testhash", {"from": account});
...
contract("Simulation", async accounts => {
    ...
    it("Start simulation", async () => {
       ...
       await showAllBalances(pki, accounts);
...
*/
  ...
  */
  ...
  it("Add attribute and sign it", async () => {
      ...
      console.log("increase allowance for facet, account[0]");
      let status = await pki.increaseAllowance(ethrust.address, 100000, {"from": accounts[0]});
      ...
      // generate TEST_DATA_BYTE_COUNT byte attribute
      let data = '#'.repeat(TEST_DATA_BYTE_COUNT);
      let attrId = await ethrust.createAttribute("IC", "0x1", data, {"from": accounts[2]});

      console.log(attrId);

      attrId = attrId.logs[0].args['attributeID'];
      //assert.equal(attrId >= 0, true);
      let expiry = new Date().setFullYear(new Date().getFullYear() + 1)
      ...
      console.log("Untrust signature")
      console.log(status);

      status = await ethrust.revokeSignature(signatureId);
      console.log("Revoke signature");
      console.log(status);
